...
export default async function Home() {
  // Busca os eventos cadastrados no seu banco de dados
  ...
  return (
    <div style={{ backgroundColor: '#050505', color: '#fff', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      {/* SEÇÃO 1: APRESENTAÇÃO DE IMPACTO (O FOGUETE) */}
      <section style={{ height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: '20px', background: 'radial-gradient(circle, #1a1a1a 0%, #000 100%)', borderBottom: '2px solid #FDB813' }}>
        ...
        <img 
          src="/logo-lza.png" 
          alt="EVENTOS LZA" 
          style={{ height: '160px', width: 'auto', marginBottom: '20px', filter: 'drop-shadow(0 0 15px #FDB813)' }} 
        />
        
        <h1 style={{ fontSize: '42px', fontWeight: '900', color: '#FDB813', fontStyle: 'italic', textTransform: 'uppercase', lineHeight: '1', margin: '10px 0' }}>
          EVENTOS LZA
        </h1>
        <h2 style={{ fontSize: '18px', color: '#fff', fontWeight: '300', letterSpacing: '4px', textTransform: 'uppercase', marginBottom: '30px' }}>
          O Foguete da Região 🚀
        </h2>
        
        <p style={{ color: '#888', maxWidth: '500px', lineHeight: '1.6', marginBottom: '40px', fontSize: '14px' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', padding: '60px 20px', backgroundColor: '#0a0a0a' }}>
        <div style={{ textAlign: 'center', padding: '30px', border: '1px solid #222', borderRadius: '25px', background: '#111' }}>
          ...
          <p style={{ fontSize: '10px', color: '#666', textTransform: 'uppercase', fontWeight: 'bold' }}>Eventos Divulgados</p>
        </div>
        <div style={{ textAlign: 'center', padding: '30px', border: '1px solid #222', borderRadius: '25px', background: '#111' }}>
      ...
      {/* SEÇÃO 3: LISTA DE EVENTOS (DINÂMICA) */}
      <section id="eventos" style={{ padding: '60px 20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
          {eventos.map((ev) => (
            <div key={ev.id} style={{ background: '#111', borderRadius: '30px', overflow: 'hidden', border: '1px solid #333', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
              ...
              <div style={{ padding: '25px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', color: '#aaa', fontSize: '14px', marginBottom: '20px' }}>
                  <span>📍 {ev.cidade}</span>
          ...
          {eventos.length === 0 && (
            <div style={{ textAlign: 'center', padding: '40px', color: '#444', border: '1px dashed #333', borderRadius: '20px' }}>
              <p>Nenhum evento cadastrado no momento.</p>
              <p style={{ fontSize: '12px' }}>Acesse /admin para cadastrar.</p>
            </div>
          )}
        </div>
      </section>

      {/* SEÇÃO 4: RODAPÉ E INSTAGRAM ATUALIZADO */}
      <footer style={{ padding: '80px 20px', textAlign: 'center', borderTop: '1px solid #222', background: '#000' }}>
        ...
        <p style={{ color: '#888', marginBottom: '30px', fontSize: '14px' }}>Siga o portal oficial nas redes sociais:</p>
        
        <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', marginBottom: '40px' }}>
          {/* LINK DO INSTAGRAM CORRIGIDO */}
          <a href="https://www.instagram.com/eventoslza.oficial/" target="_blank" style={{ color: '#FDB813' }}>
            <Instagram size={40} />
          </a>
          <a href="https://wa.me/5562994319156" target="_blank" style={{ color: '#FDB813' }}>
            <MessageCircle size={40} />
          </a>
        </div>
        
        <p style={{ fontSize: '11px', color: '#444', letterSpacing: '1px' }}>
          © 2024 EVENTOS LZA - TODOS OS DIREITOS RESERVADOS
        </p>
      </footer>
